...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	item_list = get_items(filters, additional_conditions)
	if item_list:
	...
	for d in item_list:
		...
		if d.stock_uom != d.uom and d.stock_qty:
			row.update({"amount": d.base_net_amount})
		else:
			row.update({"amount": d.base_net_amount})
	...
	for item in data:
		...
		if key not in output_dict:
			output_dict[key] = item.copy()
		else:
			output_dict[key]["amount"] = str(int(output_dict[key]["amount"]) + int(item["amount"]))
			output_dict[key]["stock_qty"] = str(int(output_dict[key]["stock_qty"]) + int(item["stock_qty"]))
		total_amount += int(item["amount"])
		total_qty += int(item["stock_qty"])
	
	output_list = list(output_dict.values())
	output_list.append({'item_name':'Total','stock_qty':total_qty,'amount':total_amount})
	return columns, output_list, None, None, None, skip_total_row
...
def get_conditions(filters, additional_conditions=None):
	...
	if (filters.get('branch')):
		conditions += f" AND `tabSales Invoice`.branch = '{filters.get('branch')}'"
	
	if (filters.get('type')):
		if (filters.get('type') == "Credit"):
			conditions += f" AND `tabSales Invoice`.is_pos = 0 "
		else:
			conditions += f" AND `tabSales Invoice`.is_pos = 1 "
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	return frappe.db.sql(
		...
		select
			...
			CASE
				WHEN `tabSales Invoice`.is_pos = 1 THEN 'Retail' ELSE 'Credit'
			END AS
				type
