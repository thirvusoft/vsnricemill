...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	company = filters.get('company') #core code
	if "company" in filters: #core code
		del filters["company"] #core code
	...
	item_condition = '' #core code
	item_list = [] #core code
	if company: #core code
		company_condition = f""" where `tabCompany Child Table`.company = '{company}' """ #core code
		item_list = [i['parent'] for i in frappe.db.sql(f"""select parent from `tabCompany Child Table` {company_condition} group by parent""", as_dict = 1)] #core code
	if item_list:
			item_condition = f""" and name IN ('{"', '".join(item_list)}') """
	return frappe.db.sql(
		"""select
